...
class SpongeSnapshotDeleteCommand : CommonSnapshotDeleteCommand<Text, User, Player, CommandSource>(), CommandCallable {

    companion object {
        val DESCRIPTION: Optional<Text> = Optional.of(Text.of(CommonSnapshotCommandNode.DELETE_DESCRIPTION))
        val USAGE: Text = Text.of(CommonSnapshotCommandNode.DELETE_USAGE)
    }
    ...
    override fun testPermission(source: CommandSource): Boolean {
        return source.hasPermission(registry.getOrDefault(DataSyncKeys.SNAPSHOT_DELETE_PERMISSION))
    }
