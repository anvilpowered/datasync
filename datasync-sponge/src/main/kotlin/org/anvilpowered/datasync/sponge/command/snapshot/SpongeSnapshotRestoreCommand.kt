...
class SpongeSnapshotRestoreCommand : CommonSnapshotRestoreCommand<Text, User, Player, CommandSource>(), CommandCallable {

    companion object {
        val DESCRIPTION: Optional<Text> = Optional.of(Text.of(CommonSnapshotCommandNode.RESTORE_DESCRIPTION))
        val USAGE: Text = Text.of(CommonSnapshotCommandNode.RESTORE_USAGE)
    }

    override fun process(source: CommandSource, arguments: String): CommandResult {
        execute(source, arguments.splitContext())
    ...
    override fun testPermission(source: CommandSource): Boolean {
        return source.hasPermission(registry.getOrDefault(DataSyncKeys.SNAPSHOT_RESTORE_PERMISSION))
    }
