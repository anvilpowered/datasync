...
class SpigotInventorySerializer1122 : SpigotInventorySerializer() {
    ...
    override fun serializeInventory(snapshot: Snapshot<*>, inventory: Inventory, maxSlots: Int): Boolean {
        ...
        try {
            ...
            for (i in contents.indices) {
                ...
                slot[Integer.toString(i)] = itemNBT
            }
    ...
    override fun deserializeInventory(snapshot: Snapshot<*>, inventory: Inventory, fallbackItemStack: ItemStack): Boolean {
        ...
        try {
            ...
            for (i in 0 until INVENTORY_SLOTS) {
                ...
                inventory.setItem(i,
                    CraftItemStack.asBukkitCopy(net.minecraft.server.v1_12_R1.ItemStack(
                        slot[i.toString()] as NBTTagCompound)
                    ))
            }
            if (inventory !is PlayerInventory) {
                inventory.setItem(44, exitWithoutSavingItemStackSnapshot)
                inventory.setItem(43, defaultFallbackItemStackSnapshot)
                inventory.setItem(42, defaultFallbackItemStackSnapshot)
                inventory.setItem(41, defaultFallbackItemStackSnapshot)
            }
        } catch (e: Exception) {
            e.printStackTrace()
            return false
