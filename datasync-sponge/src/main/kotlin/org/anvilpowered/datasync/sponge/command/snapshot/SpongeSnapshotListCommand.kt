...
class SpongeSnapshotListCommand : CommonSnapshotListCommand<Text, User, Player, CommandSource>(), CommandCallable {

    companion object {
        val DESCRIPTION: Optional<Text> = Optional.of(Text.of(CommonSnapshotCommandNode.LIST_DESCRIPTION))
        val USAGE: Text = Text.of(CommonSnapshotCommandNode.LIST_USAGE)
    }
    ...
    override fun testPermission(source: CommandSource): Boolean {
        return source.hasPermission(registry.getOrDefault(DataSyncKeys.SNAPSHOT_BASE_PERMISSION))
    }
